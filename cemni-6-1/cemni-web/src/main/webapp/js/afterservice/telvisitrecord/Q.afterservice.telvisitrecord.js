...
Q.telvisitrecord.selStores = {
    ...
    // 任务类型-新增
    taskTypeAddStore : Q.common.selDict(2107),
    // 意向价位
...
var initWin = function() {
    ...
    // 任务类型
    Q.telvisitrecord.selStores.taskTypeAddStore.removeAll();
    Q.telvisitrecord.selStores.taskTypeAddStore.load({
        callback : function() {
            var taskTypeObj = _form.findField('model.taskType');
            if (_form.findField('model.backtype').getValue() == 1) {// 新会员
                Q.telvisitrecord.selStores.taskTypeAddStore.removeAt(2);
                Q.telvisitrecord.selStores.taskTypeAddStore.removeAt(1);
                taskTypeObj.setValue(1);
            } else {
                Q.telvisitrecord.selStores.taskTypeAddStore.removeAt(0);
                taskTypeObj.setValue(3);
            }
        }
    });
...
var editFormItems = [ {
    ...
    items : [ {
                ...
                render : function(p) {
                    p.getEl().on('click', function(p) {
                        ...
                        _win.on("submit", function(r) {
                            ...
                            _form.findField('model.individCust.purposePrice').setValue(json.purposePrice);
                            _form.findField('model.individCust.purposeProduct').setValue(json.purposeProduct);
                            _form.findField('model.individCust.purposeCategory').setValue(json.purposeCategory);
                            _form.findField('model.individCust.purposeDay').setValue(Q.common.dateRenderer(json.purposeDay));
                            _form.findField('model.backtype').setValue(json.fresh);

                            Ext.getBody().loadMask();
                            Ext.Ajax.request({
                                url : dealUrl + "_isPosByIndividCustId.action",
                                method : 'post',
                                params : {
                                    id : json.individCustId
                                },
                                success : function(response) {
                                    var json = Ext.decode(response.responseText);

                                    _form.findField('model.saletype').setValue(json.data.saletype);
                                    initWin();
                                },
                                callback : function() {
                                    Ext.getBody().unmask();
        ...
        items : {
            fieldLabel : "回访人员<font color='red'>*</font>",
        ...
        items : {
            ...
            valueField : "value",
            displayField : "name",
            store : Q.telvisitrecord.selStores.taskTypeAddStore
        }
    } ]
}, {
    xtype : 'fieldset',
...
var cfg = {
    ...
    vp : {
        ...
        store : {
            idProperty : 'telVisitRecordId',
            url : dealUrl + '_getJson.action?filter_SIZE_telVisit=0&filter_EQ_expiredtype=0&filter_EQ_backzt=0',
