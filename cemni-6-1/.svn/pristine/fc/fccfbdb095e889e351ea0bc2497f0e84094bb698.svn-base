...
Ext.ux.UploadDialog.BrowseButton = Ext.extend(Ext.Button, {
    ...
    createInputFile : function() {
        ...
        // //2.x版本ext使用(<td class="x-btn-center">) swb update 20091223
        var button_container = this.el.child(".x-btn-mc"); // 3.x版本ext使用(<td
        // class="x-btn-mc">)
        // swb update
        // 20091223
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
    ...
    initComponent : function() {
        ...
        var tt = {
            ...
            "ready" : {
                ...
                "reset-queue" : [ {
                    action : [ this.resetQueue, this.fireResetQueueEvent, this.updateProgressBar ]
                } ],
                "start-upload" : [ {
            ...
            "adding-file" : {
                "file-added" : [ {
                    ...
                    action : [ this.startUpload, this.updateProgressBar, this.fireFileAddEvent ],
                    state : "ready"
                }, {
                    action : [ this.updateToolbar, this.updateProgressBar, this.fireFileAddEvent ],
                    state : "ready"
                } ]
            },
            "uploading" : {
        ...
        this.grid_panel = new Ext.grid.GridPanel({
            ...
            listeners : {
                rowdblclick : function(t) {
                    ...
                    if (!Ext.isEmpty(record.json.fileCode)) {
                        // Ext.Ajax.request({
                        // uploadUrl : 'download.action',
                        // params : {
                        // fileId : record.get('fileId')
                        // },
                        // method : "POST"
                        // })
                        // window.open('download.action?fileId='+record.get('fileId'));
                        document.location = win.downLoadUrl + record.json.fileCode;
                    }
                }
            }
        });
    ...
    fillToolbar : function() {
        ...
        if (this.isView == false) {
            ...
            /*
             * tb.x_buttons.reset = tb.addButton({ text :
             * this.i18n.reset_btn_text, tooltip : this.i18n.reset_btn_tip,
             * iconCls : "ext-ux-uploaddialog-resetbtn", handler :
             * this.onResetButtonClick, scope : this });
             */
            tb.add("-");
    ...
    updateProgressBar : function() {
        if (this.is_uploading) {
            ...
            var count = store.getCount();
            if (count > 0) {
                var collections = store.query("state", 1);
                var value = collections.length / store.getCount();
                this.progress_bar.updateProgress(value, (value * 100).toFixed(2) + '%' /* this.i18n.progress_waiting_text */);
            } else {
    ...
    updateToolbar : function() {
        if (this.isView == false) {
            ...
            if (this.is_uploading) {
                ...
                // .child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType]
                 ...
                 * tb.x_buttons.reset.enable() } else {
                 * tb.x_buttons.reset.disable() }
                 */
            }
        }
    ...
    removeFiles : function(file_records) {
        ...
        Ext.MessageBox.confirm('确认删除', '确认要删除所选附件吗?', function(btn) {
            if (btn == 'yes') {
                ...
                if (!Ext.isEmpty(delIds)) { // edit by wyj 将ids变成delIds
                    Ext.Ajax.request({
                        url : p.deleteUrl,// 'fileupload!deleteAffixsByAjax.action',
                        method : 'POST',
                        ...
                        params : {
                            fileIds : delIds // edit by wyj 将ids变成delIds
                        }
                    });
                }
            }
    ...
    updateRecordState : function(data) {
        if ("success" in data.response && data.response.success) {
            ...
            data.record.set('fileId', data.response.fileInfo.fileInfoId); // edit
            // by
            // wyj
            // }
        } else {
    ...
    fireUploadSuccessEvent : function(data) {
        this.fireEvent("uploadsuccess", this, data.record.get("fileName"), data.response, data);
    },
    ...
    onAjaxSuccess : function(response, options) {
        ...
        try {
            // var text = replaceHtml(response.responseText);// 替换<pre></pre>
            ...
            if (this.hiddenFieldId) {
                ...
                if (Ext.isEmpty(v))
                    Ext.getCmp(this.hiddenFieldId).setValue(json_response.collection[0]);
                else
                    Ext.getCmp(this.hiddenFieldId).setValue(v + "," + json_response.collection[0]);
            }
        } catch (e) {
        }
        var data = {
        ...
        if ("success" in json_response && json_response.success) {
            // 成功设置文件组id
            // edit by wyj this.base_params["fileGroupId"] =
            // data.response.data.fileGroupId;
            this.base_params["fileGroupId"] = data.response.fileInfo.fileGroup.fileGroupId;
