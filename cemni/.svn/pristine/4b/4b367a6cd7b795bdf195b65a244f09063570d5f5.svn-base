Ext.ns('Q.Templatement');
var dealUrl = path + '/Templatement';
var file_size_limit = 5;
var arrScopeStore = [];

Q.Templatement.selStores = {
    // 活动类型
...
var editFormItems = [ {
    ...
    items : [ {
        ...
        items : {
            ...
            valueField : "value",
            displayField : "name",
            store : Q.Templatement.selStores.actType,
            emptyText : '请选择',
            allowBlank : false,
            anchor : "85.6%",
            listeners : {
                select : function(c, r, i) {
                    ...
                    Q.Templatement.selStores.formAct.removeAll();
                    Q.Templatement.selStores.formAct.baseParams = {
                        "parentId" : r.get("dictId")
                    };
                    Q.Templatement.selStores.formAct.load();
        ...
        items : {
            ...
            valueField : "value",
            displayField : "name",
            store : Q.Templatement.selStores.formAct,
            emptyText : '请选择',
            allowBlank : false,
            anchor : "85.6%",
            listeners : {
                ...
                render : function(p) {
                    p.getEl().on('click', function(p) {
                        ...
                        _win.on("submit", function() {
                            ...
                            // 申请费用总金额清空
                            Ext.getCmp('applyTotalCost').setValue('');
                        });
                        _win.show();
                    });
                }
        ...
        items : {
            ...
            listeners : {
                render : function(p) {
                    ...
                    p.getEl().on('click', function(p) {
                        if (bntType != "add" || vp.editWin.formPanel.getForm().findField("model.org.type").getValue() == 5) {
                            return;
                        }
                        var _form = vp.editWin.formPanel.getForm();
        ...
        items : {
            ...
            listeners : {
                render : function(p) {
                    ...
                    var _form = vp.editWin.formPanel.getForm();
                    var series = _form.findField("model.series").getValue();
                    var isHidden = (bntType == "view" || series != "");
        ...
        items : {
            ...
            listeners : {
                render : function(p) {
                    ...
                    var _form = vp.editWin.formPanel.getForm();
                    var variety = _form.findField("model.variety").getValue();
                    var isHidden = (bntType == "view" || variety != "");
        ...
        items : {
            ...
            listeners : {
                render : function(p) {
                    p.getEl().on('mouseleave', function(p1) {
                        var bigger = vp.editWin.formPanel.getForm().findField("model.jewelleryAmountG").getValue();
                        var less = vp.editWin.formPanel.getForm().findField("model.jewelleryAmountL").getValue();
                        if (bigger > less) {
                            Q.tips(Q.color("累计珠宝额>=不能大于累计珠宝额<=！"), "red");
                            vp.editWin.formPanel.getForm().findField("model.jewelleryAmountG").setValue("");
                        }
                    });
                }
            }
...
var gridDtlIndi = {
    ...
    store : {
        ...
        url : path + "/Activity_getIndi.action",
        autoLoad : false
    },
    tbar : [ {
        id : "indiPartInBnt",
        ...
        handler : function() {
            ...
            if (totalCount > 5000) {
                ...
                window.open(path + "/Activity_export3.action?jasperFile=PerforamtdtlTrend&reportFileType=XLSX&" + Ext.urlEncode(_store.baseParams));
            }
        }
    }, {
        id : "indiAdd",
        ...
        handler : function() {
            ...
            _win.on("search", function(data) {
                ...
                Ext.Ajax.request({
                    url : path + "/Activity_getIndi.action?add=true",
                    params : data,
...
var gridDtlFra = {
    ...
    store : {
        url : path + "/Activity_getFra.action",
    ...
    tbar : [ {
        ...
        handler : function() {
            var _store = this.findParentByType(Ext.grid.GridPanel).getStore();
            window.open(path + "/Activity_export.action?jasperFile=PerforamtdtlTrend&reportFileType=XLSX&" + Ext.urlEncode(_store.baseParams));
        }
    } ]
};
...
var gridDtlPar = {
    ...
    store : {
        url : path + "/Activity_getPar.action",
        autoLoad : false,
    ...
    tbar : [ {
        ...
        handler : function() {
            var _store = this.findParentByType(Ext.grid.GridPanel).getStore();
            window.open(path + "/Activity_export2.action?jasperFile=PerforamtdtlTrend&reportFileType=XLSX&" + Ext.urlEncode(_store.baseParams));
        }
    } ]
};
...
var gridDtlExpectCost = {
    ...
    store : {
        ...
        url : path + "/Activity_getCost.action",
        sort : "expectCostId",
        dir : "desc",
        autoLoad : false
    },
...
var gridDtlgive = {
    ...
    store : {
        idProperty : "invId",
        url : path + "/Activity_getActGive.action",
        sort : "invId",
        dir : "asc",
        autoLoad : false
    },
...
var launchAct = [ {
    ...
    handler : function() {
        ...
        var edit = vp.editWin;
        edit.setFormValue(selected, this.name);
...
var cfg = {
    ...
    vp : {
        ...
        store : {
            ...
            url : path + '/Activity_getJsonAll.action?filter_EQ_template=1',
            sort : "activityId",
            dir : "desc"
        },
        listEditStateFn : [],
    ...
    editWin : {
        ...
        form : {
            ...
            setFormValueAfter : function(formPanel) {
                ...
                if (_activityType != "") {
                    var k = Q.Templatement.selStores.actType.find('value', _activityType);
                    Q.Templatement.selStores.formAct.baseParams = {
                        "parentId" : Q.Templatement.selStores.actType.getAt(k).get('dictId')
                    };
                    Q.Templatement.selStores.formAct.load({
        ...
        submitBefore : function(formPanel) {
            ...
            if (activityForm == "28" && integralReward == "" && integralReward == 0) {
                Q.tips("<font color='red'>活动形式为积分，请输入积分奖励且不能为0！</font>");
                return false;
            }
            if (activityForm == "2") {
