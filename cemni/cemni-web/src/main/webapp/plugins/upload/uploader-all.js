        ...
        UploadPanel = Ext.extend(Ext.Panel, {
            ...
            onUploadSuccess : function(file, serverData) { // 上传完毕返回事件
                ...
                if (Ext.util.JSON.decode(serverData).success) {
                    ...
                    record.set('fileId',Ext.util.JSON.decode(serverData).fileInfo.fileId);
                    record.commit();
                    
                    // 文件组ID
                    fileGroupId = Ext.util.JSON.decode(serverData).fileInfo.fileGroup.fileGroupId;
                    this.addPostParam("fileGroupId",fileGroupId);
                    //回调
